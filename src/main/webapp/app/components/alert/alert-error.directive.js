(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('nadellaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-nadellaApp-error');
                var entityKey = httpResponse.headers('X-nadellaApp-params');
                if (errorHeader) {
